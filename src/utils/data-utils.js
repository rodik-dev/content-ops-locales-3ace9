...
export function getLocalePosts(objects, locale) {
    const allPosts = getAllPosts(objects);
    return allPosts.filter((post) => post.locale === locale);
}

export function getLocalePostsSorted(objects, locale) {
    const localePosts = getLocalePosts(objects, locale);
    return sortPosts(localePosts);
}

export function getLocaleNonFeaturedPostsSorted(objects, locale) {
    const localePosts = getLocalePosts(objects, locale);
    const nonFeaturedPosts = localePosts.filter((post) => post.isFeatured !== true);
    return sortPosts(nonFeaturedPosts);
}
